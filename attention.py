...
class FeedForward(tf.keras.layers.Layer):
    ...
    def __init__(self, d_model: int, dff: int, dropout_rate: float=0.1):
        ...
        self.seq = tf.keras.Sequential([
            ...
            tf.keras.layers.Dense(d_model), # kalo ga ada fungsi aktivasi, pake yg linear
            tf.keras.layers.Dropout(dropout_rate)  # dropout untuk mencegah overfitting, overfitiing itu terjadi
            # ketika sebuah model bekerja scr bagus pada sebuah training data
            # tetapi bekerja jelek pada data yg baru atau unseen data
        ])
        self.add = tf.keras.layers.Add()
    ...
    def call(self, x: tf.Tensor) -> tf.Tensor:
        ...
        Returns:
            tf.Tensor: The output sequence of shape (batch_size, seq_length, d_model).
        """
        x = self.add([x, self.seq(x)]) # ini kenapa ga ada self.seq ???? oh astaga dia langsung di dalam self.add (:
        x = self.layer_norm(x) 
        return x